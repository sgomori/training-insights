...
RSpec.describe AnalyticalTools::GetTrainingLoad do
  ...
  # 2026-06-14 is a Sunday, so a window of whole weeks ends here and the
  # bucketing arithmetic is easy to read.
  around do |example|
    travel_to(Time.utc(2026, 6, 14, 16, 0, 0)) { example.run }
  end

  # Creates `tss` on each named day of the week beginning `week_start`.
  def train(week_start, weekdays, tss: 50.0, distance_meters: 10_000.0)
  ...
  describe "the window it reports on" do
    it "defaults to six weeks and states the window" do
      expect(payload[:period]).to include(days: 42, from: "2026-05-04", to: "2026-06-14")
    end
  ...
  describe "weekly breakdown" do
    ...
    it "reports one row per ISO week with its own volume and load" do
      ...
      weeks = payload[:weekly_breakdown]
      expect(weeks.size).to eq(2)
      expect(weeks.first).to include(
        week_start: "2026-06-01", week_end: "2026-06-07",
        activity_count: 2, distance_km: 16.0, tss: 80.0, longest_run_km: 8.0
      )
      expect(weeks.last).to include(week_start: "2026-06-08", activity_count: 3, tss: 180.0)
    end

    it "reports the week-over-week change between two complete weeks" do
      train(Date.new(2026, 6, 1), [ 0, 2 ], tss: 50.0)
      train(Date.new(2026, 6, 8), [ 0, 2, 4 ], tss: 50.0)

      expect(payload[:weekly_breakdown].last[:change_from_previous_pct]).to eq(50.0)
    end
    ...
    it "keeps the same keys on every row so the list can be scanned uniformly" do
      ...
      expect(keys.first).to include(:monotony, :strain, :change_from_previous_pct, :complete_week)
    end

    it "emits a row for a week with no training rather than closing the gap" do
      train(Date.new(2026, 6, 1), [ 0 ])

      empty_week = payload[:weekly_breakdown].last
      expect(empty_week[:week_start]).to eq("2026-06-08")
      expect(empty_week[:activity_count]).to eq(0)
      expect(empty_week[:tss]).to eq(0.0)
    end
  end

  describe "partial weeks at the window edges" do
    # A 10-day window ending Sunday 2026-06-14 starts on Friday 2026-06-05.
    let(:args) { { days: 10 } }

    it "marks the incomplete week rather than silently under-counting it" do
      weeks = payload[:weekly_breakdown]

      expect(weeks.first).to include(week_start: "2026-06-01", complete_week: false, days_in_window: 3)
      expect(weeks.last).to include(week_start: "2026-06-08", complete_week: true, days_in_window: 7)
    end

    it "does not compare a complete week against a partial one" do
      train(Date.new(2026, 6, 5), [ 0 ], tss: 30.0)
      train(Date.new(2026, 6, 8), [ 0, 2 ], tss: 60.0)

      expect(payload[:weekly_breakdown].last[:change_from_previous_pct]).to be_nil
    end

    it "leaves the derived weekly figures null on a partial week" do
      train(Date.new(2026, 6, 5), [ 0, 1, 2 ])

      partial = payload[:weekly_breakdown].first
      expect(partial[:monotony]).to be_nil
      expect(partial[:strain]).to be_nil
    end
  ...
  describe "ramp rate" do
    ...
    it "averages the week-over-week change in weekly load" do
      # 100, 150, 300, 300 TSS: +50%, +100%, 0% — a mean of 50%.
      ...
      expect(ramp[:value]).to eq(50.0)
      expect(ramp[:sample_size]).to eq(3)
      expect(ramp[:band]).to eq("aggressive build")
    end

    it "reports a negative ramp rate when load is coming down" do
      ...
      expect(payload[:ramp_rate][:value]).to be < 0
      expect(payload[:ramp_rate][:band]).to eq("reducing")
    end

    # A percentage change from zero is undefined, not zero. Counting it as no
    # change would report a return from injury as a steady block.
    it "skips a transition out of a week with no load rather than calling it flat" do
      train(Date.new(2026, 5, 25), [ 0, 2 ], tss: 50.0)
      train(Date.new(2026, 6, 1), [ 0, 2 ], tss: 50.0)
      train(Date.new(2026, 6, 8), [ 0, 2 ], tss: 50.0)

      ramp = payload[:ramp_rate]
      expect(ramp[:sample_size]).to eq(2)
      expect(ramp[:caveats]).to include(a_string_matching(/carried no load/))
    end

    it "reports no ramp rate at all with fewer than two complete weeks" do
      train(Date.new(2026, 6, 8), [ 0, 2, 4 ])
  ...
  describe "monotony" do
    ...
    it "counts rest days as zero, because a day with no run carries no load" do
      ...
      expect(payload[:weekly_breakdown].first[:monotony]).to eq(0.87)
    ...
    it "reports a higher figure for a week of near-identical days" do
      ...
      train(Date.new(2026, 6, 8), [ 0, 1, 2, 3, 4, 5 ], tss: 50.0)
      create(:activity, started_at: zone.parse("2026-06-14").change(hour: 9), tss_score: 45.0)

      expect(payload[:weekly_breakdown].first[:monotony]).to be > 2.0
    ...
    # Seven identical days, or seven rest days, both have a zero spread. Neither
    # has a defined monotony, and reporting infinity would look like a real
    # number.
    it "returns null rather than dividing by a zero spread" do
      train(Date.new(2026, 6, 8), [ 0, 1, 2, 3, 4, 5, 6 ], tss: 50.0)

      expect(payload[:weekly_breakdown].first[:monotony]).to be_nil
      expect(payload[:monotony][:value]).to be_nil
      expect(payload[:monotony][:caveats]).to include(a_string_matching(/did not vary enough to divide by/))
    end

    it "returns null for a week with no training at all" do
      expect(payload[:weekly_breakdown].first[:monotony]).to be_nil
  ...
  describe "strain" do
    ...
    it "multiplies the week's load by its monotony" do
      ...
      week = payload[:weekly_breakdown].first
      expect(week[:strain]).to eq((week[:tss] * week[:monotony]).round)
    end
  ...
  describe "load" do
    ...
    it "reports the window total as both a daily and a weekly figure" do
      train(Date.new(2026, 6, 1), [ 0, 1, 2, 3, 4, 5, 6 ], tss: 50.0)
      train(Date.new(2026, 6, 8), [ 0, 1, 2, 3, 4, 5, 6 ], tss: 50.0)
    ...
    it "says how much load is missing rather than reporting a confident total" do
      create(:activity, started_at: zone.parse("2026-06-10").change(hour: 9), tss_score: 50.0)
      create(:activity, :without_computed_metrics, started_at: zone.parse("2026-06-11").change(hour: 9))
  ...
  describe "shaping contract" do
    ...
    it "names a complete week with no training at all" do
      train(Date.new(2026, 6, 1), [ 0 ])

      expect(described_class.call(days: 14).structured_content[:notable])
