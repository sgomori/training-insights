...
| Chat answer cache key | Data version plus the runner's calendar day plus the normalised question. The day is in the key because the prompt states today's date and the cacheable questions are relative to it; an answer that outlived its day would describe an earlier one as current. The version is captured when the question is asked and travels with the job, so an answer computed before a mid-turn ingestion is filed under the corpus it saw. Decided 2026-09-06. |
