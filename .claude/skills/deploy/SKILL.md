...
- Migrations reverse cleanly (`db:rollback` then `db:migrate`) — Render runs them in the pre-deploy step and a broken one stops the deploy.
...
Watch the build in the Render dashboard. The build runs `bundle install` and asset precompilation; the pre-deploy step then runs `db:prepare` and `db:load_solid_schemas` before the new instance starts.
...
- **`Ops::MemoryReport` logs RSS, GC statistics and the worker heartbeat age hourly.** Grep the Render log stream for `memory rss_mb=` to see the trend before and after a deploy. `/up` passes with the job threads dead, so `worker_heartbeat_age_s` on that line is the only signal that chat and regeneration are still being served; a missing line means the scheduler itself is gone.
